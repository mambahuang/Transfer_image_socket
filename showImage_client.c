...
#include "cipher_b_test.h"
#include "cipher_g_test.h"
#include "cipher_r_test.h"
...
//	}
	...
	// If there are *.h for result_x, then I want to overwrite
	// the content of them.
	// This is for decrypting from another terminal device only
	for(int i=0;i<VSize;i++){
		for(int j=0;j<HSize*3;j=j+3){
			Buffer[(i*HSize*3)+j] = cipher_g[index];
			Buffer[(i*HSize*3)+j+1] = cipher_b[index];
			Buffer[(i*HSize*3)+j+2] = cipher_r[index];
...
//			}
			...
			for(int i=0;i<VSize*HSize;i++){
				result_b[i] = cipher_b[i];
				result_g[i] = cipher_g[i];
				result_r[i] = cipher_r[i];
			}
